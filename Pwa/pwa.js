...
export async function registerServiceWorker(rootPath = "") {
  ...
  const swUrl = `${rootPath}/sw.js?v=20260427-pwa-v25`.replace(/\\+/g, "/");
  ...
  await Promise.all(
    registrations.map(async (registration) => {
      ...
      if (registration.scope.endsWith(scope) && activeScript && !activeScript.includes("20260427-pwa-v25")) {
  ...
  if ("caches" in self) {
    try {
      ...
      await Promise.all(
        keys.filter((key) => key !== "mirest-pwa-v25").map((key) => caches.delete(key))
      );
    } catch {}
  }
